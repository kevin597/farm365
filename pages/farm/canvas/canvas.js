...
Page({
  ...
  data: {
    itemsNum: 6, //大转盘等分数
    itemsArc: 0, //大转盘每等分角度
    color: ["#efd691", "#ffffff"],//扇形的背景颜色交替；
    text: ["谢谢惠顾", "鸡蛋x1", "谢谢惠顾", "老母鸡X1", "谢谢惠顾", "鸡蛋x2"],//每个扇形中的文字填充
    img: ["/images/quan.jpg", "/images/quan.jpg", "/images/quan.jpg", "/images/quan.jpg", "/images/quan.jpg", "/images/quan.jpg"],
    isRotate: 0,
    navTitleText: {
  ...
  Images() {//绘制奖品图片，与绘制文字方法一致。
    ...
    let img = that.data.img;//放文字的数组
    for (let i = 0; i < Num; i++) {
      ctx.save();
      ctx.beginPath();
      ctx.translate(w1, h1);
      ctx.rotate(itemsArc * (i + 2) * Math.PI / 180);
      ctx.drawImage(img[i], -(w1 * 0.2), -(h1 * 0.6), (w1 * 0.4), (h1 * 0.2));
      ctx.restore();
    }
  },
